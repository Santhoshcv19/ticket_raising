...
import 'package:ticket_raising_management/features/tickets/presentation/widgets/attachment_view/attachment.dart';

class HmsAttachments extends ConsumerWidget {
  final List<AttachmentEntity> data;

  const HmsAttachments({super.key, required this.data});
      ...
      Row(children: [
        if (images.isNotEmpty)
          SecondaryButton(
            onTap: () => showDialog(
              context: context,
              builder: (context) => HmsAttachView(data: data, type: 1),
            ),
            elevation: 0,
        ...
        if (videos.isNotEmpty)
          SecondaryButton(
            onTap: () => showDialog(
              context: context,
              builder: (context) => HmsAttachView(data: data, type: 2),
            ),
            elevation: 0,
            color: AppColors.white,
            shape: RoundedRectangleBorder(
        ...
        if (audios.isNotEmpty)
          SecondaryButton(
            onTap: () => showDialog(
              context: context,
              builder: (context) => HmsAttachView(data: data, type: 3),
            ),
            elevation: 0,
            color: AppColors.white,
            shape: RoundedRectangleBorder(
        ...
        if (pdfs.isNotEmpty)
          SecondaryButton(
            onTap: () => showDialog(
              context: context,
              builder: (context) => HmsAttachView(data: data, type: 4),
            ),
            elevation: 0,
            color: AppColors.white,
            shape: RoundedRectangleBorder(
